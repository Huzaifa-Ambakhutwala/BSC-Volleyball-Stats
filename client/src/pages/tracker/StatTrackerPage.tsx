...
const StatTrackerPage = () => {
  ...
  return (
    <section className="py-8 bg-gray-50">
                  ...
                  <div className="flex flex-col items-start space-y-2">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {[1, 2, 3].map(setNum => {
                        ...
                        // Determine if this set is available (can be selected)
                        let isAvailable = false;
                        if (setNum === 1) {
                          isAvailable = !isLocked;
                        } else {
                          const prevSetKey = `set${setNum - 1}` as keyof typeof currentMatch.completedSets;
                          isAvailable = !!currentMatch.completedSets?.[prevSetKey] && !isLocked;
                        }
                        ...
                        if (isCurrentSet) {
                          ...
                          buttonStyle = 'bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed';
                          statusLabel = 'Locked';
                        }
                        return (
                          <div key={setNum} className="flex flex-col items-center">
                            <button
                              ...
                              onClick={() => isAvailable && handleSetChange(setNum)}
                            ...
                            <span className={`text-xs mt-1 ${isCurrentSet ? 'text-blue-600 font-semibold' : isLocked ? 'text-amber-600' : isAvailable ? 'text-green-600' : 'text-gray-400'}`}>{statusLabel}</span>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div className={`rounded-lg p-3 relative ${currentSet === 1
                      ...
                      {currentMatch.completedSets?.set1 && (
                        <div className="w-full flex justify-center">
                          <button
                            className="mt-2 text-xs text-center font-medium text-green-600 flex items-center justify-center border border-green-600 rounded px-2 py-0.5 hover:bg-green-50 transition cursor-pointer"
                            onClick={() => setSetToUnlock(1)}
                            type="button"
                          >
      ...
      {/* Admin Unlock Modal for Set */}
      {setToUnlock !== null && (
        <AdminUnlockModal
          matchId={selectedMatchId}
          setNumber={setToUnlock}
          onClose={() => setSetToUnlock(null)}
